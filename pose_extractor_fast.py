...
class PoseViTExtractor(extractor.ViTExtractor):
    ...
    def preprocess_batch(self, imgs: List[Image.Image], load_size: Union[int, Tuple[int, int]] = None) -> Tuple[torch.Tensor, List[Image.Image]]:
        batch_prep_imgs = []
        batch_scale_factors = []
        for img in imgs:
            scale_factor = 1
            ...
            prep_img = self.prep(img)[None, ...]
            batch_prep_imgs.append(prep_img)
            batch_scale_factors.append(scale_factor)

        batch_prep_imgs = torch.cat(batch_prep_imgs, dim=0)
        return batch_prep_imgs, imgs, batch_scale_factors
    
    
    
    def find_correspondences_batch(self, batch_pil_img1: List[Image.Image], batch_pil_img2: List[Image.Image], 
                                num_pairs: int = 10, load_size: int = 224, layer: int = 9, 
                                facet: str = 'key', bin: bool = True, thresh: float = 0.05) -> List[Tuple[List[Tuple[float, float]], List[Tuple[float, float]], Image.Image, Image.Image]]:

        batch_image1, _, _ = self.preprocess_batch(batch_pil_img1, load_size)
        batch_descriptors1 = self.extract_descriptors(batch_image1.to(self.device), layer, facet, bin)

        batch_image2, _, _ = self.preprocess_batch(batch_pil_img2, load_size)
        batch_descriptors2 = self.extract_descriptors(batch_image2.to(self.device), layer, facet, bin)
        
        saliency_maps1 = self.extract_saliency_maps(batch_image1.to(self.device))[0]
        saliency_maps2 = self.extract_saliency_maps(batch_image2.to(self.device))[0]
        
        
        

        results = []
        for image1_batch, image2_batch, descriptors1, descriptors2 in zip(batch_image1_batch, batch_image2_batch, batch_descriptors1, batch_descriptors2):
            # Extracting saliency maps for each image in the batch
            ...
            num_patches1 = descriptors1.shape[1] * descriptors1.shape[2]  # Assuming a [N, C, H, W] format
            image_idxs = torch.arange(num_patches1, device=self.device)
        ...
        # remove best buddies where at least one descriptor is marked bg by saliency mask.
            ...
            results.append((points1, points2, image1_batch, image2_batch))
    ...
    def find_correspondences(self, pil_img1, pil_img2, num_pairs: int = 10, load_size: int = 224, 
        ...
        img1_y_to_show = (img1_indices_to_show / num_patches1[1]).cpu().numpy()
        img1_x_to_show = (img1_indices_to_show % num_patches1[1]).cpu().numpy()
        img2_y_to_show = (img2_indices_to_show / num_patches1[1]).cpu().numpy()
        img2_x_to_show = (img2_indices_to_show % num_patches1[1]).cpu().numpy()
        points1, points2 = [], []
        for y1, x1, y2, x2 in zip(img1_y_to_show, img1_x_to_show, img2_y_to_show, img2_x_to_show):
            x1_show = (int(x1) - 1) * self.stride[1] + self.stride[1] + self.p // 2
            y1_show = (int(y1) - 1) * self.stride[0] + self.stride[0] + self.p // 2
