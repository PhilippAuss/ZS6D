...
if __name__=="__main__":
    root_dir = "./templates/ycbv"

    goal_dir = "./templates/ycbv_desc"

    gt_file = "./gts/ycbv_template_gt_new.json"
    ...
    cam_K = np.array([1066.778, 0.0, 312.9869,
    ...
    with torch.no_grad():
        ...
        for obj_id, labels in tqdm(tless_gt.items()):
            ...
            if obj_id not in ['18','13']:
                scale_fac = 1.0
                scale_div = 1.0
            
            elif obj_id in ['13', '18']:
                scale_fac = 0.001
                scale_div = 1

            obj_model = Model3D()
            model_path = os.path.join(goal_dir,"models_xyz", f"obj_{int(obj_id):06d}.ply")
            # model_path = os.path.join(goal_dir,"models_xyz", f"obj_000001.ply")
            # obj_model.load(model_path,scale=0.001)
            obj_model.load(model_path,scale=scale_fac)
            ...
            for i, label in enumerate(labels):
                ...
                # img_uv, depth_rend, bbox_gt = get_rendering(obj_model,rot_pose,t/100,ren)
                # img_uv, depth_rend, bbox_gt = get_rendering(obj_model,rot_pose,t/1000,ren)
                img_uv, depth_rend, bbox_gt = get_rendering(obj_model,rot_pose,t/scale_div,ren)
    ...
    with open("./gts/ycbv_templates_gt_with_uvmap_desc.json","w") as f:
